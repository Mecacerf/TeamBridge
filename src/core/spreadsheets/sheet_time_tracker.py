...
class SheetTimeTracker(TimeTrackerAnalyzer):
    ...
    def _setup(self, accessor: SheetsRepoAccessor):
        ...
        if version is None or not str(version).lower().startswith(
            ...
            raise SheetTimeTrackerError(
                (
                    f"Cannot load workbook '{self._file_path}' that uses "
        ...
        LOGGER.debug(
            (
                f"[Employee '{self._employee_id}'] Finished spreadsheet time tracker setup "
    ...
    def register_clock(self, event: ClockEvent) -> None:
        ...
        for col_idx in range(start_idx, self._col_last_clock_out + 1, 2):
            ...
            if cell.value is None:
                ...
                LOGGER.debug(
                    (
                        f"[Employee '{self._employee_id}'] Registered clock event '{event}' "
    ...
    def __get_month_sheet_idx(self) -> int:
        ...
        if self.data_year != self._date.year:
            raise TimeTrackerDateException(
                (
                    f"Cannot access date '{self._date}' while the file "
    ...
    def __get_clock_action_for_cell(self, cell: openpyxl.cell.Cell) -> ClockAction:
        ...
        """
        if (cell.column < self._col_first_clock_in) or (
            cell.column > self._col_last_clock_out
        ):
            raise ValueError(
                (
    ...
    def __load_workbook(self):
        ...
        LOGGER.debug(
            (
                f"[Employee '{self._employee_id}'] (Re)loaded spreadsheet '{self._file_path.name}' "
                f"(readable={self.readable})."
            )
        )
